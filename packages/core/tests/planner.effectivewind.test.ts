...
describe('effective wind in the planner', () => {
  ...
  it('reduces effective headwind below the raw forecast', () => {
    const plan = runInnerSolve(flatMicros(5), 150, headwind6, cfg);
  ...
  it('escape hatch: apply_wind_height_correction=false uses raw wind', () => {
    ...
    const plan = runInnerSolve(flatMicros(5), 150, headwind6, raw);
  ...
  it('calm wind is unchanged (0 -> 0)', () => {
    ...
    const plan = runInnerSolve(flatMicros(3), 150, calm, cfg);
  ...
  it('per-segment z0_used overrides the global roughness', () => {
    ...
    const wPlan = runInnerSolve(water, 150, headwind6, cfg);
    const uPlan = runInnerSolve(urban, 150, headwind6, cfg);
