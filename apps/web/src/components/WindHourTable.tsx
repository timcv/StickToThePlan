          ...
          <tr key={r.hour} className={`wind-row${edited.has(r.hour) ? ' edited' : ''}`}>
            ...
            <td>
              <span
                ...
                style={{ transform: `rotate(${r.dir_from_deg + 180}deg)` }}
              >
                {'↑'}
              </span>
            ...
            <td>
              {edited.has(r.hour) && (
                <button type="button" className="link" onClick={() => onReset(r.hour)}>
                  återställ
                </button>
              )}
            </td>
