...
- De-personalization before going public: make the MET Norway User-Agent contact configurable (default a generic project URL like the GitHub repo, not a personal email), and scrub home-directory paths and the maintainer's personal email from committed files (the current `build-report.md` and `weather/metNorway.ts`).
